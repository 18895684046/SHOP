...
<template>
  <div class="hd-wrp">
    <div class="hd-content">
      ...
      <Popup v-model:show="show" position="left" :style="{ height: '100%', width: '50%' }">
        <div class="pop-wrp">
          <Collapse :border="false" v-model="activeNames">
            <CollapseItem :border="false" title="测试服务" name="100">
              <CollapseItem :border="false" v-for="(service, index) of services" :title="service.name" :name="index">
                <CollapseItem 
                  v-for="subSrv of service.subServices" 
                  readonly 
                  :disabled="subSrv.disabled"
                  :border="false"
                  @click="hrefTo(subSrv.disabled, subSrv.url)"
                >
                  <template #title>
                    <div>{{ subSrv.name }}</div>
                  </template>
